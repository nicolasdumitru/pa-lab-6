...
void print_graph(Graph *graph) {
    for (int i = 1; i <= graph->vertices; i += 1) {
        for (Node *temp = graph->adjacency_lists[i]; temp != NULL;
             temp = temp->next) {
...
void dfs(Graph *graph, int vertex_nr) {
    ...
    for (Node *curr = graph->adjacency_lists[vertex_nr]; curr != NULL;
         curr = curr->next) {
...
void bfs(Graph *graph, int start) {
    ...
    while (!is_empty(queue)) {
        ...
        for (Node *temp = graph->adjacency_lists[current]; temp != NULL;
             temp = temp->next) {
